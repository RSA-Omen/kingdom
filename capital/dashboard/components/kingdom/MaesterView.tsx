...
export function MaesterView({ stats, error }: { stats: Stats | null; error: string | null }) {
  if (error) {
    return (
  ...
  return (
    <section className="surface p-6">
      ...
      <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
        <Stat icon="📚" label="Projects" value={summary.total_projects} />
        <Stat icon="🌳" label="Repos" value={summary.total_repos} />
        <Stat icon="😴" label={`Stale (>${daysStale}d)`} value={summary.stale_count} />
        <Stat icon="📦" label="Apps" value={summary.by_type.app || 0} />
      </div>

      <h3 className="mt-6 text-sm font-semibold text-[var(--color-text-primary)]">Recently Active</h3>

      {recent_activity.length === 0 ? (
        <p className="mt-3 text-sm text-[var(--color-text-secondary)] italic">No recent activity.</p>
      ) : (
        <ul className="mt-3 space-y-2">
          {recent_activity.slice(0, 6).map((item) => {
            const days = item.last_activity
              ? Math.floor((Date.now() - new Date(item.last_activity).getTime()) / (24 * 60 * 60 * 1000))
              : null;
            return (
              <li
                key={item.name}
              ...
              >
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-[var(--color-text-primary)]">{item.name}</p>
      ...
      <p className="mt-5 text-[10px] text-[var(--color-text-tertiary)]">
        Last indexed{" "}
        {new Date(stats.timestamp).toLocaleString("en-GB", { dateStyle: "medium", timeStyle: "short" })}{" "}
        · — The Maester
      </p>
    </section>
  );
