type MasterOfWorksData = {
  ...
  services: Array<{ emoji: string; name: string; status: string; isHealthy: boolean }>;
  timestamp: string;
};

export function MasterOfWorksView({ data, error }: { data: MasterOfWorksData | null; error: string | null }) {
  if (error) {
    return (
  ...
  return (
    <section className="surface p-6">
      ...
      {services.length === 0 ? (
        <p className="mt-3 text-sm text-[var(--color-text-secondary)] italic">No services checked.</p>
      ) : (
          ...
          {services.map((service) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-xs font-medium text-[var(--color-text-primary)]">{service.name}</p>
                  <p className="text-[10px] text-[var(--color-text-tertiary)]">{service.status}</p>
                </div>
              </div>
            </li>
          ))}
...
function ResourceStat({
  ...
  return (
    <div className="border border-[var(--color-border)] rounded p-2 text-center">
      ...
      <p className={`mt-1 text-lg font-semibold tabular-nums ${color}`}>{value.toFixed(1)}%</p>
