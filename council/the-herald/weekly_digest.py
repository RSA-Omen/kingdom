...
RECENT_ADDITIONS = [
    "The Guild Board — the king's single pane of glass at `/guild-board`, now showing live data. Aggregates every incomplete Asana task (triaged or untriaged) with Capital DB incidents (deduped by fingerprint, with occurrence-count badges). Smart triage: untriaged Asana, High-priority work, and frequent or critical incidents bubble to Needs Attention; routine in-progress items stay In Flight. Sidebar village counts derived from live data. Server-component page refreshes every 30s.",
    "The Lord Chamberlain — first full end-to-end production cycle. The agent now polls Asana every 5 minutes, classifies every new task with Claude Sonnet (bug/feature/support/internal-tool/it-request/rd-idea/unclear + High/Medium/Low priority + confidence), routes to the correct Digital or IT Support section, sets custom fields, tags `lc-triaged`, posts a structured triage comment, and updates the Guild Board within 30 seconds.",
    "Lord Chamberlain Telegram is now action-only — pings the king only on High-priority work, bug classifications, or unclear classifications (where judgement is needed). Routine Medium/Low priority work is visible on the Guild Board, not pushed to Telegram. Reduces notification noise from one-per-task to one-per-actionable-task.",
    "Guild Board Chain view — clicking any triage row now drills into a project lifecycle chain. Fetches Asana task stories (section changes, comments, current status) and synthesises them into a vertical spine diagram: diamond milestones, circle phases, cloud comms events, with expandable subtrees showing comment threads. Metadata sidebar shows stakeholder, village, started date, last comms, and a stuck-days alert when waiting. Capital DB incidents get a simplified three-node chain (Reported → Open → Resolved).",
]
