#!/usr/bin/env bash
# The Scout — Ralph-style loop runner.
# Each iteration spawns a fresh `claude -p` to investigate ONE bug-tagged issue.
# Loop exits when the queue is empty (Scout prints NO_WORK) or when ./STOP exists.
#
# Usage:
...
while (( i < MAX_ITERS )); do
  ...
  if printf '%s\n' "$last5" | grep -q '^NO_WORK$'; then
    echo "Queue empty — Scout reported NO_WORK." | tee -a "$LOG"
    break
  fi

  if ! printf '%s\n' "$last5" | grep -qE '^INVESTIGATED #[0-9]+$'; then
    echo "WARNING: iteration $i did not end with INVESTIGATED or NO_WORK. Halting to avoid runaway." | tee -a "$LOG"
    break
  fi
done

printf '\nThe Scout — loop ended after %d iteration(s).\n' "$i" | tee -a "$LOG"
